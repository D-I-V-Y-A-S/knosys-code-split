                ...
                if detail.get("itemType") == "SharedParagraph":
                    ...
                    if title_value and value:
                        ...
                        if "<ac:image>" in value:
                            ...
                            if image_item_id:
                                image_page_title = f"Image - {image_item_id}"
    ...
    try:
        ...
        for a_tag in soup.find_all("a", attrs={"data-itemid": True}):
            ...
            if detail.get("itemType") == "Document":
                ...
                main_page = get_page_by_title(space_key, title)
                print("main_page",main_page)
                if not main_page:
                    print(title,"hello no page")
                    create_page(space_key, title,"To be updated!")
                    
                # Step 6: Replace the <a> with Confluence link macro
                link_macro = f'''
    <ac:link>
    <ri:page ri:content-title="{title}" ri:space-key="{space_key}" />
...
def extract_content_from_fields(child):
    try:
        ...
        for macro in image_macros:
            ...
            image_page_title = f"Image - {item_id}"
...
"""
        ...
        if not existing_hidden_page:
            with open("test_output_1.html", "w", encoding="utf-8") as f:
                f.write(hidden_text)
            create_page(shared_paragraph_space_key, clean_title, hidden_text)
...
<td>{Document_Id}</td>
<td>{title}</td>
<td><a href="https://elabor8demo.atlassian.net/wiki/spaces/{space_key}/pages/{page_id}">{page_id}</a></td>
</tr>
'''
